...
if __name__ == '__main__':
    ...
    var1 = result.xs(key='speed', axis='columns', level=1)
    var2 = result.xs(key='angle', axis='columns', level=1)
    
    ax[0].plot(t_plot, var1.values)
    ax[0].set_ylabel('Speed')
    ax[0].set_xlabel('Time [s]')
    ax[0].grid(True)
    ax[1].plot(t_plot, var2.values)
    ...
    ax3.plot(result[('Global', 't')], p3.values)
    ax3.set_title('Pe')
